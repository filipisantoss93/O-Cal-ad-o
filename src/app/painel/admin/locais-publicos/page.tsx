...
export default async function PublicPlacesAdminPage({
  ...
  const { supabase } = await requireAdmin("/painel/admin/locais-publicos");
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link
          href="/painel/admin/locais-publicos?novo=1"
              ...
              return (
                <article key={place.id} className="rounded-2xl border border-line bg-surface p-5 shadow-sm">
                  ...
                  <div className="mt-4 flex gap-2">
                    <Link href={`/painel/admin/locais-publicos?local=${place.id}`} className="inline-flex min-h-10 flex-1 items-center justify-center rounded-xl bg-ink px-4 text-sm font-black text-white">Editar</Link>
