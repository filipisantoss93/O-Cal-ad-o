...
export default async function UnclaimedBusinessesAdminPage({ searchParams }: PageProps) {
  ...
  const pageHref = (nextPage: number) => {
    ...
    return `/painel/admin/perfis-nao-reivindicados${value ? `?${value}` : ""}`;
  };
  ...
  return <div>
    ...
    <section className="mt-7 rounded-[2rem] border border-line bg-surface p-5 shadow-sm sm:p-8">
      ...
      {hasFilters && <div className="mt-4 flex flex-wrap items-center gap-3"><p className="text-sm font-bold text-muted">{total} perfil{total === 1 ? "" : "s"} encontrado{total === 1 ? "" : "s"}.</p><Link href="/painel/admin/perfis-nao-reivindicados" className="text-sm font-black text-brand-dark underline">Limpar filtros</Link></div>}
    </section>
