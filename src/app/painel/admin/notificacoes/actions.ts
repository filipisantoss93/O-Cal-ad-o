...
export async function markAdminNotificationRead(formData: FormData) {
  ...
  if (!Number.isSafeInteger(id) || id <= 0) redirect("/painel/admin/notificacoes?erro=invalid");
  const { error } = await supabase.from("admin_notifications")
    .update({ read_at: new Date().toISOString() }).eq("id", id).eq("recipient_id", user.id);
  if (error) redirect("/painel/admin/notificacoes?erro=update");
  revalidatePath("/painel/admin/notificacoes");
}

export async function resolveAdminMessage(formData: FormData) {
  const { supabase } = await requireAdmin("/painel/admin/notificacoes");
  const id = Number(formData.get("id"));
  const kind = String(formData.get("kind"));
  if (!Number.isSafeInteger(id) || id <= 0 || !["support", "report"].includes(kind)) redirect("/painel/admin/notificacoes?erro=invalid");
