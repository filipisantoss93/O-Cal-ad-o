...
export async function reviewBusinessClaimAction(formData: FormData) {
  ...
  if (decision !== "approve" && decision !== "reject") redirect("/painel/admin/reivindicacoes?erro=" + encodeURIComponent("Decisão de reivindicação inválida."));
  let errorMessage = "";
  try {
    ...
    revalidatePath("/painel/admin/reivindicacoes"); revalidatePath("/painel/admin"); revalidatePath("/painel");
    if (slug) { revalidatePath(`/loja/${slug}`); revalidatePath(`/reivindicar/${slug}`); }
  } catch (error) { errorMessage = messageFor(error); }
  if (errorMessage) redirect(`/painel/admin/reivindicacoes?erro=${encodeURIComponent(errorMessage)}`);
  redirect(`/painel/admin/reivindicacoes?sucesso=${encodeURIComponent(decision === "approve" ? "Reivindicação aprovada e estabelecimento transferido ao responsável." : "Reivindicação rejeitada.")}`);
}
...
export async function reviewBusinessListingRequestAction(formData: FormData) {
  ...
  const decision = formString(formData, "decision");
  const slug = formString(formData, "slug");
  if (!["resolve", "reject", "remove"].includes(decision)) redirect("/painel/admin/reivindicacoes?erro=" + encodeURIComponent("Decisão de correção inválida."));
  let errorMessage = "";
  try {
    ...
    if (error) throw new Error(error.message);
    revalidatePath("/"); revalidatePath("/buscar"); revalidatePath("/painel/admin/reivindicacoes"); revalidatePath("/painel/admin/perfis-nao-reivindicados");
    if (slug) revalidatePath(`/loja/${slug}`);
  } catch (error) { errorMessage = messageFor(error); }
  if (errorMessage) redirect(`/painel/admin/reivindicacoes?erro=${encodeURIComponent(errorMessage)}`);
  redirect(`/painel/admin/reivindicacoes?sucesso=${encodeURIComponent(decision === "remove" ? "Perfil removido da publicação e solicitação concluída." : decision === "reject" ? "Solicitação rejeitada." : "Solicitação marcada como resolvida.")}`);
}
