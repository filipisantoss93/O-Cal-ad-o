  ...
  return <div className="space-y-8">
    <header className="rounded-[2rem] bg-ink p-6 text-white sm:p-9">
      ...
      <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
        ...
        <Link href="/admin" className="inline-flex min-h-11 items-center rounded-xl bg-white px-5 text-sm font-black text-ink">Abrir fila de lojas</Link>
      </div>
    </header>
    ...
    <nav aria-label="Período dos indicadores" className="flex flex-wrap items-center gap-2">
      ...
      {([7, 30, 90] as const).map((value) =>
        <Link key={value} href={`/admin/dashboard?periodo=${value}`}
    ...
    <section aria-labelledby="valores">
      ...
      <dl className="mt-3 grid grid-cols-2 gap-2 sm:mt-4 sm:gap-4 xl:grid-cols-4">
        ...
        <Metric label="Aguardando pagamento" value={money(m.ads.awaiting_payment_cents)} detail={`${number(m.ads.awaiting_payment)} campanhas ainda não pagas`} href="/admin/destaques?status=pending" />
      </dl>
    ...
    <section aria-labelledby="pendencias">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 sm:mt-4 sm:gap-3 xl:grid-cols-3">
        {[
          ...
          { label: "Mensagens de suporte", count: m.inbox.support_open, href: "/admin/notificacoes?tipo=support" },
