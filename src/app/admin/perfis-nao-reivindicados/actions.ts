...
export async function createUnclaimedBusinessAction(formData: FormData) {
  ...
  try {
    ...
    redirect(`/admin/perfis-nao-reivindicados?erro=${encodeURIComponent(messageFor(error))}`);
  }
  redirect("/admin/perfis-nao-reivindicados?sucesso=Perfil%20criado%20com%20sucesso");
}
...
export async function updateUnclaimedBusinessAction(formData: FormData) {
  ...
  const editPath = `/admin/perfis-nao-reivindicados/${businessId}/editar`;
  ...
  try {
    ...
    const suffix = returnTo === "/admin/reivindicacoes" ? `&return_to=${encodeURIComponent(returnTo)}` : "";
