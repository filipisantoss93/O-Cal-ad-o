...
export const metadata: Metadata = { title: "Sinalizar informações", robots: { index: false, follow: false } };
...
export default async function ReportPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><SiteHeader /><main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 sm:py-16">
    <Link href={`/loja/${slug}`} className="text-sm font-bold text-brand-dark hover:underline">← Voltar ao cadastro</Link>
    <h1 className="mt-5 text-3xl font-black text-ink sm:text-4xl">
      {business.listingType === "public_place" ? "Sinalizar informação sobre" : "Denunciar"} {business.name}
    </h1>
