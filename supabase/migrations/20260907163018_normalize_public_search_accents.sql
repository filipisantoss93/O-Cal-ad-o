...
create index if not exists businesses_search_normalized_idx
...
create index if not exists catalog_items_search_normalized_idx
on public.catalog_items using gin (
  pg_catalog.to_tsvector(
    'simple',
    public.search_normalize(coalesce(name, '') || ' ' || coalesce(description, ''))
  )
)
  ...
  matched as (
    ...
    where b.city_id = p_city_id
      ...
      and (
        ...
        ) @@ pg_catalog.websearch_to_tsquery('simple', params.normalized_query)
        or pg_catalog.to_tsvector(
          'simple',
          public.search_normalize(coalesce(c.name, ''))
        ) @@ pg_catalog.websearch_to_tsquery('simple', params.normalized_query)
        or exists (
          select 1
        ...
        or exists (
          ...
          where ci.business_id = b.id
            ...
            and pg_catalog.to_tsvector(
              'simple',
              public.search_normalize(coalesce(ci.name, '') || ' ' || coalesce(ci.description, ''))
            ) @@ pg_catalog.websearch_to_tsquery('simple', params.normalized_query)
        )
      )
  )
