#!/usr/bin/env python3
"""Lê o extrato PBF de SP do OpenStreetMap: apenas vias/áreas com amenity=charging_station.
...
class Stations(osmium.SimpleHandler):
    ...
    def way(self, node):
        ...
        self.counts["all_charging_ways"] += 1
        coords = [(nd.location.lat, nd.location.lon) for nd in node.nodes if nd.location.valid()]
        if not coords:
            self.counts["invalid_coordinates"] += 1
            return
        lat = sum(pair[0] for pair in coords) / len(coords)
        lon = sum(pair[1] for pair in coords) / len(coords)
        if not (-25.55 <= lat <= -19.60 and -53.35 <= lon <= -44):
            self.counts["outside_sp_bbox"] += 1
            return
        ...
        self.stations.append({
            "external_id": f"osm:way:{node.id}", "name": name,
            ...
            "access_type": access, "source_url": f"https://www.openstreetmap.org/way/{node.id}",
            "source_checked_at": None, "source_license": "© OpenStreetMap contributors · ODbL 1.0",
        })
        self.counts["eligible"] += 1

def main():
    if len(sys.argv) != 3: raise SystemExit("Uso: prepare-osm-sp-charging.py SP.osm.pbf OUTPUT_DIR")
    handler = Stations()
    handler.apply_file(sys.argv[1], locations=True, idx='sparse_mem_array')
    ...
    manifest = {**handler.counts,"parts":(len(stations)+79)//80,
       "source":"OpenStreetMap São Paulo PBF extrato openstreetmap.fr 2026-09-21",
       "license":"ODbL 1.0","url":"https://download.openstreetmap.fr/extracts/south-america/brazil/southeast/sao-paulo-latest.osm.pbf"}
