#!/usr/bin/env python3
"""Extrai eletropostos do Brasil com licença CC BY 4.0 do export público OCM.
...
def main():
    ...
    for path in sorted(root.glob("OCM-*.json")):
        ...
        try:
            ...
            if not (-34.0 <= lat <= 5.5 and -74.1 <= lon <= -34.0):
                raise ValueError("outside Brazil bounding box")
        ...
        station = {
            ...
            "name": title, "city": town, "state_hint": validated(address.get("StateOrProvince"), 80), "street": street, "number": number,
