#!/usr/bin/env python3
"""Extrai dados CC0 das fontes Easy Volt e YellotMob do export público OCM.
...
def main():
    ...
    for path in sorted(root.glob("OCM-*.json")):
        ...
        try:
            ...
            continue
        if record.get("DataProviderID") not in {40, 57}:
            counts["other_provider"] += 1
            continue
        ...
        station = {
            ...
            "source_license": ("Easy Volt · CC0" if record.get("DataProviderID") == 40 else "YellotMob · CC0"),
        }
    ...
    manifest = {**counts, "parts": (len(stations) + 79) // 80, "source": "Open Charge Map ocm-export data/BR", "provider_ids": [40, 57], "license": "CC0", "snapshot_date": "2026-04-23", "states": ["SP"]}
